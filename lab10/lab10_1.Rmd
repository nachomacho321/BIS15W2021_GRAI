...
```{r}
elephants %>% 
  ggplot(aes(x=age, fill(therm), y=height)) + geom_point() + geom_smooth(method=lm, se=F) +
  labs(title = "Elephant Age vs. Height",
       x = "Age",
       y = "Height") +
...
homerange %>% 
  ...
  geom_bar()+
  labs(title="Counts of ectotherms and endotherms",
       x=NULL,
       y="count")
```
...
homerange %>% 
  ...
  geom_point()+
  labs(title="Elephants Homerange",
       x="Log 10 Mass",
       y= "Log 10 Homerange")
```
